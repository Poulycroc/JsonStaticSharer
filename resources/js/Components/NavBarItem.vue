...
import { isNil } from "@poulycroc/js-utils";
...
const is = computed(() => {
  return props.item.href || props.item.to
    ? "a"
    : "div";
});
...
const rendNavItemLink = computed(() => {
  const { to, href } = props.item
  if (isNil(to) && isNil(href)) return null
  return !isNil(to) ? route(to) : href
});
